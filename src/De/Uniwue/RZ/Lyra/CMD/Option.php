...
class Option
{
    ...
    private $aliases = array(); /* aliases for this argument */
    private $value = null; /* mixed */
    private $description; /* string */
    private $required = false; /* bool */
    private $needs = array(); /* set of other required options for this option */
    private $optionals = array(); /* set the other optionals for this option*/
    ...
    private $file_require_exists; /* bool require that the file path is valid */
    private $file_allow_globbing; /* bool allow globbing for files */
    private $run; /* string command to run */
    ...
    public function __construct($name)
    {
        if (!is_int($name) && empty($name)) {
            throw new \Exception(sprintf(
                'Invalid option name %s: Must be identified by a single character or an integer',
                $name
            ));
        }
    ...
    /**
    * Set an option as optional
    *
    * @param string $option Option name
    *
    * @return Option
    */
    public function setOptionals($option){
        if (!is_array($option)) {
            $option = array($option);
        }
        foreach ($option as $opt) {
    ...
    /**
    * Returns the current set of this option's optional parameters
    *
    * @return array
    */
     ...
     *
     * @return bool|array True if requirements met, array if not found
     */
    ...
    {
        if ($this->isBoolean() && !is_bool($value) && !is_null($value)) {
            throw new \Exception(
                sprintf('Boolean option expected for option %s, received %s value instead', $this->name, $value)
            );
        }
